import { FriendSchema, PlaceFullSchema, UserSchema } from "@/constants/types";
...
export const useStore = create<StoreSchema>((set, get) => ({
  ...
  mockAddFav: () => {
    // Randomize
    const keys = Object.keys(likedPlacesData);
    const rndKey = Math.floor(Math.random() * keys.length);

    set((state) => ({
      ...state,
      favourites: [...state.favourites, likedPlacesData[rndKey]],
    }));
  },
